...
**Version:** 1.0.0-draft
**Status:** In progress. See `final/BUILD-PLAN.md` for chapter ownership and `final/session-log.md` for the rolling build log.
...
I run six agentic systems on a single Mac Mini. Together they produce daily competitive briefs across 22 verticals, run an automated outreach pipeline for a healthcare directory, schedule and triage a control plane for several other agents, ghostwrite my LinkedIn presence under a voice profile, and paper-trade a crypto strategy I am no longer working on. They share one human operator and zero API spend beyond a subscription. The interesting part is not that they all run. The interesting part is that they did not always.
...
| `iip.db` | Project database | Keep source-specific database names out of public body copy. |
| `IIP_ENHANCEMENTS.md` | Session log | Mention source path only in citation index. |
...
These seven files are the provenance surface. A new user who runs `verify-integrity.py` against them gets a clean local check of "this is the unmodified v1.0 release." See Chapter 27 (Governance) and the attribution architecture established in BUILD-PLAN section 1. The license split (CC BY 4.0 for docs, Apache 2.0 for code) is the standard combination for a kit that mixes prose and reusable code samples.
...
### Appendix E. Citation Index

> **Status:** Pre-staged by Kiro (WQ-016). Claude to finalize in Phase F with any additional claims.

Every claim in this manual that references a specific number, a specific system behavior, or a specific production incident is traced here to a verifiable source file.

**Source system:** Industry Intelligence Platform (IIP), running on a Mac Mini. All source files are in the `masterportfolio/Industry-Intelligence-Platform/` repository unless otherwise noted.

---

**Foreword: "Six agentic systems on a single Mac Mini"**
Source: `Cameron Brain/Mac Mini/Agent Architecture.md` -- "The Three Agents" section documents the multi-agent architecture. The six systems are: IIP, IV Directory, Paperclip, Hannah/OpenClaw, LinkedIn pipeline, Hydra.

**Foreword: WAL contention incident (HG-2 resolved in Loop 4)**
Source: `masterportfolio/Industry-Intelligence-Platform/IIP_ENHANCEMENTS.md`, Session ID `2026-05-04-cold-start-contention-fix`. The session documents: "18 of 22 verticals lost overnight because the LLM cache opened its own SQLite handle on every call -- a single-writer architecture that was not actually single-writer." Specific commits: 560c6f7, 7bc85c7, 0c3f22e.

**Chapter 8: "100,000 to 250,000 tokens saved over 50 sessions"**
Source: `Cameron Brain/Mac Mini/Industry Intelligence Platform/Token Optimization Strategies - IIP Reference.md`, Layer 1 section: "A well-maintained CLAUDE.md + STATE.md pair replaces 2,000-5,000 tokens of 're-explain the project' at session start. Over 50 sessions, that's 100,000-250,000 tokens saved."

**Chapter 12: LLM response cache (sha256 key, 24h TTL)**
Source: `masterportfolio/Industry-Intelligence-Platform/src/iip/llm/cache.py` -- `_canonicalize`, `_key_for`, `get_cached`, `put_cached` functions. The `llm_cache` table in `data/iip.db` is the production implementation.

**Chapter 12: Input-fingerprint cache**
Source: `masterportfolio/Industry-Intelligence-Platform/src/iip/pipelines/vacuum_precompute.py` -- `_process_node` function. Uses `sha256(summaries + sector_context + prior_context)` as the cache key.

**Chapter 13: Cache attribution sidecar**
Source: `masterportfolio/Industry-Intelligence-Platform/src/iip/llm/cache.py` -- `llm_cache_attribution` table, `savings_by_role()` function, `find_duplicate_key_misses()` function.

**Chapter 15: Event-driven dispatch**
Source: `masterportfolio/Industry-Intelligence-Platform/src/iip/events/` -- queue, handlers, worker. Producer hooks at `NewsStore.save_article`, `MaterialEventAnalyst._persist_event`.

**Chapter 15: Activity-gated batch work**
Source: `masterportfolio/Industry-Intelligence-Platform/.orchestration/iip-sector-sync.py` -- `sector_has_recent_activity` function. Queries `material_events`, `article_tags`, `company_reports.updated_at`.

**Chapter 17: Model routing table (MODEL_ASSIGNMENTS)**
Source: `masterportfolio/Industry-Intelligence-Platform/src/iip/config.py` -- `MODEL_ASSIGNMENTS` dict. Routes `event_detection`, `news_synthesis`, `financial_analysis` to Ollama; routes `global_synthesis`, `executive_summary`, `competitive_landscape` to ClaudeCodeProvider.

**Chapter 17: Provider cascade and circuit breaker**
Source: `masterportfolio/Industry-Intelligence-Platform/src/iip/llm/provider.py` -- `ClaudeCodeProvider.generate` retry loop. Cascade: `["claude_code", "anthropic", "groq", "gemini"]`. Circuit breaker: 3 consecutive failures, 10-minute cooldown.

**Chapter 18: BaseAgent interface contract**
Source: `masterportfolio/Industry-Intelligence-Platform/src/iip/agents/base.py` -- `BaseAgent` class with `_build_prompt()`, `_parse_response()`, `_validate_output()`. 23 active agents all extend this class.

**Chapter 18: generate_structured() auto-retry**
Source: `masterportfolio/Industry-Intelligence-Platform/src/iip/llm/structured.py` -- `generate_structured()` function. One-shot retry with schema error injected into prompt.

**Chapter 22: Intelligence lineage graph**
Source: `masterportfolio/Industry-Intelligence-Platform/src/iip/lineage.py` -- `intelligence_lineage` table schema, `record()`, `record_batch()`, `stats()` functions. Subject types, source types, and relation types are defined here.

**Chapter 23: pipeline_runs table and track_phase**
Source: `masterportfolio/Industry-Intelligence-Platform/scripts/scheduled/pipeline_log.py` -- `pipeline_runs` table DDL, `track_phase` context manager, `run_phase` bash helper.

**Chapter 24: Quality Immune System -- four-layer implementation**
Source:
- Layer 1 (merge-preserve): `masterportfolio/Industry-Intelligence-Platform/src/iip/quality/preservation.py` -- `merge_preserve()` function, `SECTION_FLOORS` dict.
- Layer 2 (DB trigger): `masterportfolio/Industry-Intelligence-Platform/src/iip/quality/preservation.py` -- `install_db_triggers()` function.
- Layer 3 (quality ledger): `masterportfolio/Industry-Intelligence-Platform/src/iip/quality/preservation.py` -- `ensure_quality_ledger()`, `report_quality_ledger` table.
- Layer 4 (scheduled sweep): `masterportfolio/Industry-Intelligence-Platform/scripts/scheduled/quality_sweep.py`.

**Chapter 24: "24 public-company reports regressed to 9-25% of historical content size"**
Source: `masterportfolio/Industry-Intelligence-Platform/IIP_ENHANCEMENTS.md`, Reliability Pattern 7 section: "24 public-company reports (META, NVDA, AMZN, BRK.B, BAC, INTC, CRM, V, HD, plus Materials-sector tickers) regressed to 9-25% of their historical content size. Discovered by Cameron manually reviewing the Verizon page, not by any automated signal."

**Appendix C: Anti-pattern "Bulk nightly re-sweep of unchanged data"**
Source: `masterportfolio/Industry-Intelligence-Platform/IIP_ENHANCEMENTS.md` Session 01 and `Cameron Brain/Mac Mini/knowledge/Patterns for Durable Agentic Systems.md` Token-Efficiency Pattern 1 (Event-driven over time-based bulk sweeps): "The nightly news classifier re-processed a 7-day window every run, re-touching thousands of already-classified articles."

**Appendix C: Anti-pattern "Cache hit rate assumed good"**
Source: `Cameron Brain/Mac Mini/knowledge/Patterns for Durable Agentic Systems.md` Token-Efficiency Pattern 7 (Attribution table for cache savings): "Token efficiency is measurable, not assumed."

**Appendix C: Anti-pattern "Narrative content regression"**
Source: `Cameron Brain/Mac Mini/knowledge/Patterns for Durable Agentic Systems.md` Reliability Pattern 7 (Narrative content quality immune system): full incident description with dates and affected tickers.

---

**Chapter 3: Three-Era Maturity Model (Demo, Ramp-up, Durable)**
Source: `Cameron Brain/Mac Mini/knowledge/Patterns for Durable Agentic Systems.md`, "Three-Era Maturity Model" section. The IIP project's own progression through Era 01 (2026-Q1, end-to-end demo), Era 02 (2026-Q1 to Q2, scheduled overnight runs), and Era 03 (2026-Q2 onward, schema validation, lineage, observability) is the reference case for the model.

**Chapter 4: Two-Tier Output Model (Observable vs Synthetic)**
Source: `masterportfolio/Industry-Intelligence-Platform/docs/vacuum-strategy/vacuum-agent-prompt-spec.md`. The two-tier distinction is enforced in IIP's vacuum detection pipeline: Tier 1 candidates ("observable gaps any analyst could spot") are logged but not surfaced unless volume is anomalous; Tier 2 candidates ("synthesized from cross-domain evidence") are surfaced and tracked.

**Chapter 5: X+Y=Do Z formula, Verizon example**
Source: `masterportfolio/Industry-Intelligence-Platform/docs/vacuum-strategy/spotlight-generation-guide.md`. The fiber-network plus inference-latency formulation is drawn from IIP's Verizon spotlight pipeline. The X plus Y schema is enforced at the prompt boundary in the spotlight generator's output schema: `existing_asset`, `unmet_need`, `specific_action`.

**Chapter 27: "Act, do not ask" governance rule**
Source: Cameron's global operator-contract conventions, distilled from one year of `~/.claude/CLAUDE.md` operating practice across the six-system Mac Mini portfolio. The red-lines list (destructive git, paid actions, external publishing, data-dropping migrations, auth and secret changes) is hook-enforced where possible across IIP, IV Directory, Paperclip, and Hannah.

**Chapter 27: Blast-radius framework (local-reversible, shared, external)**
Source: `Cameron Brain/Mac Mini/knowledge/Patterns for Durable Agentic Systems.md`, governance discipline section. The three-category split is the live operating policy across the Mac Mini agentic ecosystem.

**Chapter 30: "If the answer is in a file, do not summarize it"**
Source: `Cameron Brain/Mac Mini/Industry Intelligence Platform/Token Optimization Strategies - IIP Reference.md`, Layer 4 ("Direct load before summary"). Documented as the single most common AI misapplication in the Mac Mini ecosystem.

**Chapter 30: "When the data is structured, use a query, not retrieval"**
Source: Same reference, Layer 4 ("Structured data before RAG"). IIP uses Chroma only for unstructured knowledge (session logs, design decisions, lessons learned); structured queries against `iip.db` are direct SQL, not RAG.

---

**Notes on framework chapters without IIP implementation citations.** Chapters 1, 2, and 6 are synthesis chapters that do not point at a specific implementation file. Chapter 1 (compound vs reset) is a diagnostic, not an implementation; the symptoms list is observed across the broader AI tooling landscape, not a specific IIP module. Chapter 2 (AI Tool Evaluation Framework) is a procurement-grade framework derived from the same operating-system thinking that produced the rest of the manual. Chapter 6 (Promise Map) is a connector chapter pointing to other chapters that themselves carry the citations. Their lack of a code-level citation is not a gap; it reflects the nature of the chapters as frame, not implementation.

---

*Citation index complete. 22 entries total: 15 Kiro pre-staged in Phase C, 7 Claude added in Phase F.*
...
*Compound AI Operating Standards v1.0.0-draft. Foreword complete; remainder in progress. Build coordinated in `final/session-log.md`.*
