...
- No long dashes found under `final/`.
...
All checks were run from `final/compound-ai-starter-kit/`.
...
The foreword placeholder block at HG-2 must be resolved before the document ships. Two candidates are documented in the placeholder. Kiro's recommendation: use the WAL contention story from IIP_ENHANCEMENTS.md Session 2026-05-04.
...
**Part IV (Chapters 17-20):** Accurate. The model routing table is consistent with IIP's MODEL_ASSIGNMENTS. The BaseAgent interface contract matches `src/iip/agents/base.py`. The three-role model (Architect/Operator/Builder) matches `Agent Architecture.md`.
...
- `iip.db` / SQLite -> "your persistent store"
...
**Subject:** `final/FIELD-GUIDE.md` v1.0.0-draft, 1,949 lines, 31 chapters, 6 appendices
...
Adapted from the `ivd-content-adversary` skill's 0-100 scoring axes:
