...
Runs the ten-step ship gate on a candidate release. Either every check passes and the release ships, or any failure blocks the ship until fixed. The kit ships its own metabolism: this skill exists because v2.3.1 shipped with a verify-integrity failure that v2.4.0 had to correct. `release-captain` makes that class of failure mechanically catchable before the push, not after.
