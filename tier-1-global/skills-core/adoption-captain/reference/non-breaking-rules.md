...
the current kit version (v2.5.0). The agent then:

1. Lists files that are new in v2.5.0 (safe to add)
2. Lists files that have changed (propose replacement per file)
3. Lists files that are in the existing kit but not in v2.5.0 (surface to
...
and overwrites it entirely to install v2.5.0.
